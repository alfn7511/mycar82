<?php 
	...
	$sql = "
		...
		SELECT DISTINCT bid_idx, info.bid_code, bid_name, bid_phone, bid_maker, bid_car_name, bid_car_age, bid_car_num, bid_mileage, bid_capacity, bid_sales_area, bid_transmission, bid_fuel 
		   	...
		   	,proc_idx, proc_price, proc_state, proc_regdate
		...
		FROM bid_info info inner join  bid_process proc on info.bid_code = proc.bid_code
		WHERE bid_state = '$bid_state' AND bid_del_yn = 'N' AND deal_id = '".$_COOKIE['deal_id']."';
	";
	...
	foreach ( $res as $rs ){
		...
		if($bid_state==1){
			$__tmp['bid_result'] = 	"
				...
				<span class=\"txt\">현재 ".$rs['bid_max_price']."만원</span>			
			";	
		}else if($bid_state==2){
	...
	$sql = "
		...
		FROM bid_process proc inner join bid_info info on proc.bid_code = info.bid_code
		WHERE deal_id = '".trim($_COOKIE['deal_id'])."' AND bid_del_yn = 'N'
		GROUP BY bid_state
	";
