<?php 
	...
	$bid_state = (isset($_GET['bid_state'])) ? trim($_GET['bid_state']) : 0;
	
	$output['bid_info'] = array();
	$sql = "
		
		SELECT DISTINCT bid_idx, info.bid_code, bid_name, bid_phone, bid_maker, bid_car_name, bid_car_age, bid_car_num, bid_mileage, bid_capacity, bid_sales_area, bid_transmission, bid_fuel 
		    ...
		    ,TIMEDIFF(date_add(bid_regdate, interval bid_request_time hour), now()) AS bid_timer
		FROM bid_info info inner join  bid_zzim zzim on info.bid_code = zzim.bid_code
		WHERE bid_state = '$bid_state' AND bid_del_yn = 'N' AND zzim.deal_id = '".$_COOKIE['deal_id']."';
	";
	...
	$sql = "
		...
		FROM bid_info info inner join  bid_zzim zzim on info.bid_code = zzim.bid_code
		WHERE zzim.deal_id = '".trim($_COOKIE['deal_id'])."' AND bid_del_yn = 'N'
		GROUP BY bid_state
	";
	$res_state_cnt = $dbc->getRecordList( $sql );
	
	if($res_state_cnt){
		$bid_state_cnt = array(0,0,0);
		...
		$output['bid_state_cnt'] = "
			<div class='state'>
				...
				<a href='?bid_state=0'><span>요청".$bid_state_cnt[0]."건</span></a>
				<a href='?bid_state=1'><span>진행".$bid_state_cnt[1]."건</span></a>
				<a href='?bid_state=2'><span>완료".$bid_state_cnt[2]."건</span></a>
			</div>
		";	
	}
